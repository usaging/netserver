...
Connection::Connection(EventLoop *loop, std::unique_ptr<Socket> clientsock, uint16_t sep , const string &sepstr)
    : loop_(loop), clientsock_(std::move(clientsock)), disconnect_(false), clientchannel_(new Channel(loop_, clientsock_->fd()))
{
    ...
    inputbuffer_.setsep(sep, sepstr);
    outputbuffer_.setsep(sep, sepstr);
}
